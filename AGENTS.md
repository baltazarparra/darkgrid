...
10. **Three target platforms — portrait is PRIMARY.** iPhone portrait Safari (~393px wide) and Android Chrome PWA portrait (manifest locks `pwa/orientation=1`) are the primary targets; safe areas handled by `ControlsHud`. The `PortraitGuard` autoload (layer 128) shows the "gire o dispositivo" overlay when a PHONE is in LANDSCAPE (web + landscape + short side < `Constants.PHONE_SHORT_SIDE_MAX`) — landscape gameplay only exists on tablet/desktop (short side ≥ 640; arena zoom capped at 2.0x in `arena_manager.gd`). Run `/validate-platforms` before committing any UI, camera, or safe-area change.
