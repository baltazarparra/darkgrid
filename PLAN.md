...
| KI-011 | Baixa | ✅ Resolvida | `main_menu.gd:35` iterava os botões com `for button: Button in [...]`, mas `GithubLink` é `LinkButton` (irmão de `Button` sob `BaseButton`, não subclasse) — a conversão tipada vira `Nil`, estoura `SCRIPT ERROR` em todo boot e o som de hover do link nunca era conectado. Corrigido tipando o loop como `BaseButton`. |
