...
| KI-012 | Baixa | Parcial | Hierarquia de escala dos bosses: a PROPORÇÃO na arena foi corrigida fiel à lore via `sprite_scale`/offset de pés por cena (Saci 1.8 < Caipora ≈ Curupira 2.0 < Jesuíta 2.7/machados 2.8 ≈ caçador < Boitatá 3.0 < Mula 3.5; contrato em `test_boss_scale_proportions.gd`). Segue pendente: arte legada 48×48 upscalada gera texels maiores que os dos comuns (112×112) — resolve nas sessões de redesign de cada chefe no pipeline premium (`gen_inimigos.py`, canvas ≥128, herdando as alturas visuais do contrato), um por sessão; no MAPA os bosses seguem 48×48. |
