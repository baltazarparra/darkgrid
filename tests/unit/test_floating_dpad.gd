...
func test_resolve_action_dead_zone_returns_empty() -> void:
	assert_eq(FloatingDpadScript.resolve_action(Vector2(6.0, 0.0), DEAD, ""), "")
	assert_eq(FloatingDpadScript.resolve_action(Vector2.ZERO, DEAD, "ui_right"), "")

func test_resolve_action_all_cardinals() -> void:
	assert_eq(FloatingDpadScript.resolve_action(Vector2(30.0, 0.0), DEAD, ""), "ui_right")
	assert_eq(FloatingDpadScript.resolve_action(Vector2(-30.0, 0.0), DEAD, ""), "ui_left")
	assert_eq(FloatingDpadScript.resolve_action(Vector2(0.0, -30.0), DEAD, ""), "ui_up")
	assert_eq(FloatingDpadScript.resolve_action(Vector2(0.0, 30.0), DEAD, ""), "ui_down")
...
func test_axis_hysteresis_keeps_current_axis_near_diagonal() -> void:
	...
	assert_eq(FloatingDpadScript.resolve_action(Vector2(20.0, 22.0), DEAD, "ui_right"), "ui_right")
	# …mas quando o eixo vertical domina com folga (> bias de 1.25x), troca.
	assert_eq(FloatingDpadScript.resolve_action(Vector2(20.0, 30.0), DEAD, "ui_right"), "ui_down")

func test_same_axis_flips_immediately() -> void:
	# Wiggle ←→ sem levantar o dedo (sequência do Curupira): sinal inverte na hora.
	assert_eq(FloatingDpadScript.resolve_action(Vector2(-25.0, 0.0), DEAD, "ui_right"), "ui_left")
...
func test_screen_rect_tracks_pad_position() -> void:
	_pad.begin_touch(Vector2(300.0, 300.0))
	var rect: Rect2 = _pad.get_screen_rect()
